...
export class BuscadorUnidadComponent {
  ...
  protected readonly buscar = (
    texto$: Observable<string>,
  ): Observable<UnidadPanorama[]> => {
    const escrito$ = texto$.pipe(debounceTime(150), distinctUntilChanged());
    const clics$ = this.clic$.pipe(
      filter(() => !this.typeahead().isPopupOpen()),
    );
    return merge(escrito$, this.foco$, clics$).pipe(
      map((texto) => this.filtrar(texto)),
    );
  };

  protected readonly formato = (unidad: UnidadPanorama): string =>
  ...
  private filtrar(texto: string): UnidadPanorama[] {
    ...
    const filtradas = q
      ? lista.filter((u) => normalizar(this.indexable(u)).includes(q))
      : lista;
  ...
  private etiqueta(unidad: UnidadPanorama): string {
    const base = unidad.torre
      ? `${unidad.torre} · ${unidad.identificador}`
      : unidad.identificador;
